<?php
    ...
    for($i=0; $i<count($suites); $i++) {
        ...
        $pdf->Cell(50,6,'Number Of Beds',1,0,'C');
        $pdf->Cell(50,6,'Number Of Units',1,0,'C');
        $pdf->Cell(50,6,'Number Of Units Remaining',1,1,'C');

        $pdf->SetFont('Arial','B',9);

        $pdf->Cell(40,6,$suites[$i][1],1,0,'C');
        $pdf->Cell(40,6,$suites[$i][2],1,0,'C');
        $pdf->Cell(50,6,$suites[$i][3],1,0,'C');
        $pdf->Cell(50,6,$suites[$i][6],1,0,'C');
        $pdf->Cell(50,6,$suites[$i][7],1,0,'C');
        $pdf->Cell(50,6,$suites[$i][8],1,1,'C');
